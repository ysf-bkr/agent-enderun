...
describe("Hermes Locking & Self-Healing Stress Test", () => {
    let tempDir: string;
    ...
    it("should handle concurrent message delivery and respect locks", async () => {
        ...
        const backendMsgsFile = path.join(messagesDir, "backend.json");
        const content = fs.readFileSync(backendMsgsFile, "utf8").trim();
        const lines = content.split("\n");
        
        expect(lines.length).toBe(5); // All messages delivered despite concurrency
    });
