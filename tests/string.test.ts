...
describe("String Utilities", () => {
    ...
    describe("slugifyName", () => {
        it("should convert to lowercase, replace special chars with hyphens and trim hyphens", () => {
            expect(slugifyName("Enderun App Core")).toBe("enderun-app-core");
            expect(slugifyName("Agent-Manager_123!")).toBe("agent-manager-123");
            expect(slugifyName("---Leading-Trailing---")).toBe("leading-trailing");
        });

        it("should fallback to enderun-app for empty or invalid slugs", () => {
            expect(slugifyName("")).toBe("enderun-app");
            expect(slugifyName("!!!")).toBe("enderun-app");
            expect(slugifyName(null as any)).toBe("enderun-app");
        });
    });

    describe("titleCase", () => {
        it("should format string to Title Case and handle delimiters", () => {
            expect(titleCase("enderun-app-core")).toBe("Enderun App Core");
            expect(titleCase("agent_manager_test")).toBe("Agent Manager Test");
            expect(titleCase("  multiple   spaces  ")).toBe("Multiple Spaces");
        });

        it("should fallback to Enderun App for empty input", () => {
            expect(titleCase("")).toBe("Enderun App");
            expect(titleCase(null as any)).toBe("Enderun App");
        });
    });
});
