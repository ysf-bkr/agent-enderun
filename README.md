...
4. **Çoklu Ajan Kaosu:** **13** farklı uzman yapay zeka ajanı, olay tabanlı asenkron bir haberleşme protokolü olan **Hermes Message Broker** üzerinden koordine edilir. Ajanlar birbirine rastgele müdahale edemez; tüm görevler bir İş Dağılım Grafiği (DAG) üzerinden komuta edilir.
...
> [!IMPORTANT]
> **Mimari Hatırlatma:** Model Context Protocol (MCP) araçları, **insan geliştirici (kullanıcı) tarafından manuel olarak çalıştırılmaz.** MCP sunucu araçları; yapay zeka ajanlarının (Claude Code, Gemini, Grok, Cursor) projeniz üzerinde cerrahi kod yazma, durum sorgulama ve Hermes mesajlaşma gibi eylemleri **otonom olarak çalıştırabilmesi** için arka planda yapay zeka motorlarına bağlanır. İnsan geliştirici ise komuta sürecini CLI (`status`, `trace:new` vb.) üzerinden yönetir.
...
*   **`plan`**: `docs/` dizini altındaki tüm `.md` dosyalarını okur, başlıkları ve görevleri tespit ederek planlama yapar.
...
4. **Symmetric Orchestration (Hermes Protocol):** **13** specialized expert agents communicate via an asynchronous, event-driven message broker (**Hermes**), preventing chaotic, uncoordinated AI behavior.
...
### 💎 Strategic Platform Synergy (Six Major Powers)
...
* **✍️ Cursor (Implementer / Developer):** The **Code Worker**. IDE integration for writing, fixing, and refactoring tasks.
* **💡 Codex (GitHub Copilot - Coding Assistant):** The **Assistant Developer**. VSCode integration for code completion, suggestions, and rapid prototyping.
...
> [!IMPORTANT]
> **Architectural Note:** Model Context Protocol (MCP) tools are **not meant to be executed manually by the human developer (user).** Instead, the MCP server runs in the background, allowing AI Agents (Claude Code, Gemini, Grok, Cursor) to **autonomously execute** our custom tools (like surgical editing, state reading, or Hermes messaging) on your codebase. The human developer controls and monitors this command loop using standard CLI commands (`status`, `trace:new`, etc.).
...
#### 4. Cursor Adapter
Initialize Cursor IDE rules and MCP integration:
```bash
npx agent-enderun init cursor
```

#### 5. Codex (GitHub Copilot) Adapter
Set up VSCode and GitHub Copilot instructions:
```bash
npx agent-enderun init codex
```
...
*   **`plan`**: Read all `.md` files from `docs/` directory and create planning tasks for AI agents.
